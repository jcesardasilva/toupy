...
from toupy.io.filesrw import read_tiff, converttiffto16bits
from toupy.utils import mask_borders, convert_to_mu, convert_to_rhoe, convert_to_rhom
...
print(
    "I have found {} tiff files for the beta tomogram".format(len(list_tiff_files_beta))
)
print("Reading first slice to get size")
nslices = slice_numbers[-1] - slice_numbers[0] + 1
...
for idx, ii in enumerate(list_tiff_files_beta[slice_numbers[0] : slice_numbers[-1]]):
    ...
    if histtype == "rhoe_mu":
        print(
            "Converting to linear attenuation coefficient: {}".format(idx + 1), end="\r"
        )

...
print('I have found {} folder: \n{} \n{}'.format(len(tiff_folders),
                                                 os.path.split(tiff_folders[0])[1], os.path.split(tiff_folders[1])[1]))
...
for ii in tiff_folders:
    if fnmatch.fnmatch(ii, '*beta*'):
        ...
        tiff_beta_wcard = os.path.join(
            ii, 'tomo_beta_filter_type_*_freqscl_*.tif')
...
print('I have found {} tiff files for the delta tomogram'.format(
...
for idx, ii in enumerate(list_tiff_files_delta[slice_numbers[0]:slice_numbers[-1]]):
    ...
    tomogram_delta[idx] = convert_tiff_value(
        ii, low_cutoff_delta, high_cutoff_delta)
    if histtype == 'rhoe_mu':
...
print('I have found {} tiff files for the beta tomogram'.format(
...
for idx, ii in enumerate(list_tiff_files_beta[slice_numbers[0]:slice_numbers[-1]]):
    ...
    tomogram_beta[idx] = convert_tiff_value(
        ii, low_cutoff_beta, high_cutoff_beta)
    if histtype == 'rhoe_mu':
        print('Converting to linear attenuation coefficient: {}'.format(
            idx+1), end='\r')
...
aspectratio = 1.0*(xmax*factor_scale_beta - 0) / \
    (1.0*ymax*factor_scale_delta - 0)
...
cax2d = (ax2dhisto.imshow(np.log(H+np.spacing(1)),
                          extent=[xmin*factor_scale_beta, xmax*factor_scale_beta,
                                  ymin*factor_scale_delta, ymax*factor_scale_delta],
                          vmin=0,
...
axhistbeta.set_xlim([betaplotlims[0]/factor_scale_beta,
                     betaplotlims[1]/factor_scale_beta])  # x.min(), x.max() )
axhistbeta.xaxis.set_major_locator(plticker.MultipleLocator(0.2e-6))
axhistdelta.set_ylim([deltaplotlims[0]/factor_scale_delta,
                      deltaplotlims[1]/factor_scale_delta])  # y.min(), y.max()  )
...
axhistbeta.xaxis.set_major_locator(
    plticker.MultipleLocator(0.2e-6))  # MaxNLocator(7))

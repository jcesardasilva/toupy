    ...
    if not params[u"pyhst"]:
        ...
        if params[u"tomo_algorithm"] == u"FBP":
            ...
            tomogram_align = mod_iradon(
                sinogram_align,
                theta=theta,
                output_size=sinogram_align.shape[0],
        ...
        print(
            "The full tomographic reconstrucion has not been done. Saving on the calculated slice."
        )
        a = input(
            "Save the (e) edf projections from the batch results or (v) vol file in HDF5? Type s to skip (e/[v]/s)"
        ).lower()
        if a == "" or a == "v":

...
params[u'filtertomo'] = 0.4     # Frequency cutoff (between 0 and 1)
params[u'circle'] = True
params[u'vmin_plot'] = None#0.5e-5
params[u'vmax_plot'] = -1e-3#None
...
params[u'tomo_algorithm'] = u'FBP' #u'FBP' # u'SART' # SART does not work with derivatives
params[u'opencl'] = True
params[u'pyhst'] = False
#=========================
...
if __name__=='__main__':
    ...
    aligned_projections, theta, shiftstack, outkwargs = L('aligned_projections.h5')
    ...
    if not params[u'pyhst']:
        ...
        if params[u'tomo_algorithm'] == u'FBP':
            t0 = time.time()
            tomogram_align = mod_iradon(sinogram_align,theta=theta,output_size=sinogram_align.shape[0],filter_type=params['filtertype'],derivative=params[u'derivatives'],freqcutoff=params['filtertomo'])
            ...
            if initial_guess:
                #with initial guess
                reconstruction_sart = iradon_sart(sinogram_align, theta = theta, image = tomogram_align,relaxation = relaxation_params)
            else:
                #without initial guess
                reconstruction_sart = iradon_sart(sinogram_align, theta = theta,relaxation = relaxation_params)
            iteration_num = 2
            for ii in range(iteration_num):
                print('Iteration {}'.format(ii+1))
                tomogram_align = iradon_sart(sinogram_align, theta = theta,image = reconstruction_sart,relaxation = relaxation_params)
        ...
        im1 = ax1.imshow(tomogram_align*circle,cmap=params['colormap'],interpolation='none',vmin=vmin_plot,vmax=vmax_plot)
        ...
        if str(a)=='' or str(a)=='y':
            ...
            tomogram = np.zeros((aligned_projections.shape[1],slice_size[0],slice_size[1]))
            plt.close('all')
            plt.ion()
            for ii in range(nslices):#num_projections):#sorted(frames):
                ...
                tomogram[ii] = mod_iradon(sinogram,theta=theta,output_size=sinogram.shape[0],filter_type=params['filtertype'],derivative=params[u'derivatives'],freqcutoff=params['filtertomo'])
                if params['circle']:
                    tomogram[ii]=tomogram[ii]*circle
                pf=time.time()-p0
                print('Time elapsed: {:4.02f} s'.format(pf))#time.time()-p0))
            ...
            S('tomogram.h5',tomogram,theta,shiftstack)
        elif str(a)=='n':
        ...
        a = input('Save the (e) edf projections from the batch results or (v) vol file in HDF5? Type s to skip (e/[v]/s)').lower()
        if a == '' or a =='v':
            del aligned_projections
            print('Saving .vol into HDF5')
            filename = 'volfloat/{}.vol'.format(params[u'samplename'])
            ...
            file_wcard = 'volnfp/{}*.edf'.format(params[u'samplename'])

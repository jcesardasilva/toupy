...
if __name__ == "__main__":
    ...
    while True:
        ...
        if np.sign(params["rot_axis_offset"]) == -1:
            print(
                "Initial guess of the rotation axis offset : {}".format(
                    params["rot_axis_offset"]
                )
            )
            # np.transpose(aligned_diff[:,slice_num,:])
            sinogram_prealign = np.pad(
                sinogram_prealign,
                ((0, 2 * abs(params["rot_axis_offset"])), (0, 0)),
                "constant",
                constant_values=0,
            )
            ...
            sinogram_prealign = np.pad(
                sinogram_prealign,
                ((2 * abs(params["rot_axis_offset"]), 0), (0, 0)),
                "constant",
                constant_values=0,
            )
        ...
        if params[u"opencl"]:
            ...
            B = None
            tomogram_prealign = mod_iradon2(
                sinogram_prealign,
                theta=theta,
                output_size=sinogram_prealign.shape[0],
                filter_type=params["filtertype"],
                derivative=params["derivatives"],
                freqcutoff=params["filtertomo"],
            )
        else:
            tomogram_prealign = mod_iradon(
                sinogram_prealign,
                theta=theta,
                output_size=sinogram_prealign.shape[0],
                filter_type=params["filtertype"],

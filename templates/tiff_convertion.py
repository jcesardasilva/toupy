...
if __name__=='__main__':
    ...
    if tomo_type =='delta':
        ...
        tomogram,theta,deltastack,voxelsize, kwargs = save_or_load_tomogram('tomogram.h5', **inputparams) #pathfilename=inputkwargs['path_filename'],h5name='tomogram.h5')
    elif tomo_type =='beta':
        inputkwargs[u'phaseonly'] = False
        inputkwargs[u'amponly'] = True
        tomogram,theta,deltastack,voxelsize, kwargs = save_or_load_tomogram('tomogram_amp.h5', **inputparams) #pathfilename=inputkwargs['path_filename'],h5name='tomogram_amp.h5')
    else:
    ...
    tiff_subfolder_name='TIFF_{}_{}_freqscl_{:0.2f}_{:d}bits'.format(tomo_type,params['filtertype'],params['filtertomo'],inputkwargs[u'bits'])
    tiff_folder_path = os.path.join(aux_path,tiff_subfolder_name)
    if not os.path.isdir(tiff_folder_path):#aux_path+'/unwrapped_phases.h5'):
        print('Folder {} does not exists and will be create'.format(tiff_subfolder_name))
        os.makedirs(tiff_folder_path)
    else:
        print('Folder exists:{}'.format(tiff_folder_path))
        userans = raw_input('Do you want to overwrite TIFFs in this folder ([y]/n)?').lower()
    ...
    for ii in range(tomogram.shape[0]):
        ...
        imgtiff = np.fliplr(tomogram[ii].copy())-low_cutoff # fliplr to have same orientation as holoct
        ...
        # Writing to file
        tiff=libtiff.TIFF.open(tiff_folder_path+'/tomo_{}_filter_type_{}_freqscl_{:0.2f}_{:04d}.tif'.format(tomo_type,params['filtertype'],params['filtertomo'],ii),mode='w')
        tiff.write_image(imgtiff)
        tiff.close()
        print('writing slice {} of {:03d}'.format(ii,tomogram.shape[0],tiff_folder_path))
    ...
    fid.write('# low_cutoff + [(high_cutoff-low_cutoff)*tiff_image/(2^{:d}-1)] \n'.format(inputkwargs[u'bits']))
    fid.close()
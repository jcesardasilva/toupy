...
if __name__ == '__main__':
    ...
    if tomo_type == 'delta':
        ...
        tomogram, theta, deltastack, voxelsize, kwargs = save_or_load_tomogram(
            'tomogram.h5', **inputparams)  # pathfilename=inputkwargs['path_filename'],h5name='tomogram.h5')
    elif tomo_type == 'beta':
    ...
    if ext == '.ptyr':
        ...
        aux_wcard = re.sub(
            u'_subtomo\d{3}_\d{4}', '', os.path.splitext(filename)[0])
    else:
        raise IOError(
    ...
    if not os.path.isdir(tiff_folder_path):
        print('Folder {} does not exists and will be create'.format(
            tiff_subfolder_name))
        os.makedirs(tiff_folder_path)
    else:
        print('Folder exists:{}'.format(tiff_folder_path))
        userans = raw_input(
            'Do you want to overwrite TIFFs in this folder ([y]/n)?').lower()
    ...
    for ii in range(tomogram.shape[0]):
        ...
        # Writing to file
        tiff = libtiff.TIFF.open(tiff_folder_path+'/tomo_{}_filter_type_{}_freqscl_{:0.2f}_{:04d}.tif'.format(
            tomo_type, params['filtertype'], params['filtertomo'], ii), mode='w')
        tiff.write_image(imgtiff)
        tiff.close()
        print('writing slice {} of {:03d}'.format(
            ii, tomogram.shape[0], tiff_folder_path))

...
# (150,650)      # Window inside regstack used for alignment delta = 200; % Window should be centered in x to ensure compliance with iradonfast limsx = [1+delta size(regstack,2)-delta];
params[u"roiy"] = (1, 1913)
params[u"showmovie"] = False
# in the case of showmovie = False, choose the projections you want to plot
params[u"projections"] = np.arange(0, 1)
...
if __name__ == "__main__":
    ...
    if params["derivatives"]:
        a = input(
            "Do you want to proceed with the derivative calculation? ([y]/n) :"
        ).lower()
        if str(a) == "" or str(a) == "y":
            # compute projection derivatives
            aligned_diff = np.empty_like(
                aligned[:, roiy[0] : roiy[-1], roix[0] : roix[-1]]
            )
            ...
            for ii in range(nprojs):
                ...
                print(
                    "Done. Time Elapsed {:.02f}s. Estimated remaining time: {:.02f}s.".format(
                        delta_time, delta_time * rem_time
                    )
                )
            a = input(
                "Do you want to display the calculated derivatives projections? ([y]/n): "
            ).lower()
        else:
            raise SystemExit(
                "Derivative projections have NOT been calculated. Choose an new ROI."
            )
    else:
    ...
    if str(a) == "" or str(a) == "y":
        ...
        fig = plt.figure(5)
        ax1 = fig.add_subplot(111)
        im1 = ax1.imshow(aligned_diff[0], cmap="bone", vmin=-0.2, vmax=0.2)
        plt.ion()

...
pathfilename1 = tkFileDialog.askopenfilename(
    initialdir='.', title='Please, load the reference frame ...')  # '/data/id16a/inhouse1/commissioning/')
...
image1 = np.angle(rmphaseramp(
    np.exp(1j*img1), weight=None, return_phaseramp=False))
image2 = np.angle(rmphaseramp(
    np.exp(1j*img2), weight=None, return_phaseramp=False))
...
precision = (
    input("Do you want to use pixel(1) or subpixel(2) precision registration?[1] "))
if (precision == str(1) or precision == ''):
    ...
    print("Detected pixel offset [y,x]: [{:g}, {:g}]".format(
        shift[0], shift[1]))
    ...
    print("Time elapsed: {:g} s".format(end-start))
    print("Detected subpixel offset [y,x]: [{:g}, {:g}]".format(
        shift[0], shift[1]))
else:
    print("You must choose between 1 and 2")
    raise SystemExit

print("Shift = [%g, %g], Error= %g, DiffPhase = %g" %
...
startfsc = time.time()
FSC2D = FSCPlot(image1, offset_image2.real, 'onebit',
                params['thick_ring'], transv_apod=params['transv_apod'], axial_apod=params['axial_apod'])
...
#a = intersection
print("------------------------------------------")
print("| Resolution is estimated to be %.2f nm |" %

...
if __name__ == '__main__':
    ...
    if params[u'oldfileformat']:
        # 'recons/bone2_hoptyfluo_nfptomo_nfpxct/aligned_projections_proj.h5'
        filename = 'aligned_projections.h5'
        # in principle this one also works for the modern file format
        aligned_projections, theta, pixelsize = load_data_FSC(
            filename, **inputparams)
    else:
        aligned_projections, thetaunsort, deltastack, pixelsize, kwargs = save_or_load_data(
    ...
    t0 = time.time()
    tomogram1 = mod_iradon(sinogram_align1, theta=theta1, output_size=sinogram_align1.shape[0], filter_type=params[
    ...
    t0 = time.time()
    tomogram2 = mod_iradon(sinogram_align2, theta=theta2, output_size=sinogram_align2.shape[0], filter_type=params[
                           'filtertype'], derivative=params[u'derivatives'], freqcutoff=params['filtertomo'])
    print('Calculation done. Time elapsed: {} s'.format(time.time()-t0))
    ...
    startfsc = time.time()
    FSC2D = FSCPlot(tomogram1, tomogram2, 'halfbit',
                    params['thick_ring'], apod_width=params['transv_apod'])
    ...
    save_or_load_FSCdata('FSC2D.h5', normfreqs, T, FSC2Dcurve,
                         tomogram1, tomogram2, theta, voxelsize, **inputparams)
    ...
    if a == '' or a == 'y':
        ...
        for idx, ii in enumerate(range(limsyFSC[0], limsyFSC[-1])):
            ...
            t0 = time.time()
            sinogram_align1 = sinogram_align[:, 0::2]
            tomogram1[idx] = mod_iradon(sinogram_align1, theta=theta1, output_size=sinogram_align1.shape[0],
                                        filter_type=params['filtertype'], derivative=params[u'derivatives'], freqcutoff=params['filtertomo'])
            print('Calculation done. Time elapsed: {} s'.format(time.time()-t0))
            print('Calculating second slice...')
            t0 = time.time()
            sinogram_align2 = sinogram_align[:, 1::2]
            tomogram2[idx] = mod_iradon(sinogram_align2, theta=theta2, output_size=sinogram_align2.shape[0],
                                        filter_type=params['filtertype'], derivative=params[u'derivatives'], freqcutoff=params['filtertomo'])
            print('Calculation done. Time elapsed: {} s'.format(time.time()-t0))
        ...
        startfsc = time.time()
        FSC3D = FSCPlot(tomogram1, tomogram2, 'halfbit',
                        params['thick_ring'], apod_width=params['transv_apod'])
        normfreqs, T, FSC3Dcurve = FSC3D.plot()
        endfsc = time.time()
        print("Time elapsed: {:g} s".format(endfsc-startfsc))
        print("The voxelsize of the data is {:.02f} nm".format(
            voxelsize[0]*1e9))
        ...
        save_or_load_FSCdata('FSC3D.h5', normfreqs, T, FSC3Dcurve,
                             tomogram1, tomogram2, theta, voxelsize, **inputparams)

...
print("The pixelsize of the first recons is {:04.2f} nm".format(
    pixelsize1[0]*1e9))
print('The dimensions in pixels of the data are {}'.format(img1.shape))
print('Therefore, the dimensions in microns are ({:.2f},{:.2f})'.format(
...
for ff in frames:
    try:
        ...
        im1 = ax1.imshow(image1, cmap='bone',
                         interpolation=interp_type, vmin=smin, vmax=smax)
        ...
        im2 = ax2.imshow(image2, vmin=vmean-permean, vmax=vmean+permean,
                         cmap='gray', interpolation=interp_type)  # origin='lower',
...
if savetif:
    ...
    tifffilenamep = os.path.splitext(os.path.basename(filename))[
        0]+'_phase.tif'
    tiffp = libtiff.TIFF.open(tifffilenamep, mode='w')
    tiffp.write_image(image1)
    tiffp.close()

    tifffilenamea = os.path.splitext(os.path.basename(filename))[
        0]+'_amplitude.tif'

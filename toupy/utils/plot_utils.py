...
def _createcanvasvertical(proj, lims, vertfluctinit, vertfluctcurr,
    ...
    im_array = [im11, im21, im22, im31, im31a,
                im31b, im32, im32a, im32b, im41, im42]
...
class RegisterPlot:
    ...
    def updatehorizontal(self):
        ...
        self.ax11.set_title(
            'Reconstruced slice. Iteration {}'.format(self.count))
...
def _animated_image(stack_array, *args):
    ...
    im = ax.imshow(stack_array[0, limrow[0]:limrow[-1],
                               limcol[0]:limcol[-1]], cmap='bone', animated=True)
    # ~ title = ax.text(0.5,1.05,"",fontsize=20,bbox={'facecolor':'w','alpha':0.5,'pad':5},
    # ~ transform=ax.transAxes,ha='center')
    title = ax.text(0.5, 1.05, "", fontsize=20,
                    transform=ax.transAxes, ha='center')
    #~ plt.tight_layout()
...
class ShowProjections:
    ...
    def show_projections(self, obj, probe, idxp):
        ...
        if idxp == 0:
            ...
            self.im1 = self.ax1.imshow(self.objamp,
                                       ...
                                       vmax=self.cmax)
            self.ax1.set_title(
                'Object magnitude - projection {}'.format(self.idxp+1))
            self.im2 = self.ax2.imshow(self.objph,
                                       ...
                                       vmax=np.pi)
            self.ax2.set_title(
                'Object Phase - projection {}'.format(self.idxp+1))
    ...
    def update_show(self):
        ...
        self.ax3.set_title(
            'Probe (1st mode) - projection {}'.format(self.idxp+1))
        self.fig.show()
        plt.pause(0.001)

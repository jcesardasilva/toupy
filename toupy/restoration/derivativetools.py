...
__all__ = ["gradient_axis","chooseregiontoderivatives", "derivatives", "derivatives_sino"]
...
def chooseregiontoderivatives(stack_array, **params):
    ...
    while True:
        ...
        if str(ans)=='' or str(ans)=='y':
            ...
            while True:
                deltax = eval(input('Enter new value from edge of region to edge of image in x: '))
                if isinstance(deltax, int):
                    roix = (deltax, stack_array.shape[2] - deltax)

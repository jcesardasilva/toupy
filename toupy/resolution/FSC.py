...
def load_data_FSC(h5name, **params):
    ...
    aux_wcard = re.sub(r'_subtomo\d{3}_\d{4}_\w*',
    ...
    if oldfileformat:
        ...
        with h5py.File(h5file, 'r') as fid:
            ...
            for ii in range(nprojs):
                print('Loading projection {} out of {}'.format(
                    ii+1, nprojs), end='\r')
                stack_proj[ii] = fid['aligned_projections_proj/{}'.format(
                    key_list[ii])][()]
...
class FourierShellCorr:
    ...
    def apodization(self):
        ...
        if self.ndim == 2:
            ...
            windowsag = np.array(
                [window1 for ii in range(self.nr)]).swapaxes(0, 1)
            #win2d = np.rollaxis(np.array([np.tile(windowaxial,(1,1)) for ii in range(n[0])]),1).swapaxes(1,2)
            win2d = np.array([np.tile(windowaxial, (1, 1))
                              for ii in range(self.ns)])
            window = np.array([np.squeeze(
                win2d[:, :, ii])*windowsag for ii in range(self.nc)]).swapaxes(0, 1).swapaxes(1, 2)
        else:
            print('Number of dimensions is different from 2 or 3. Exiting...')
            raise SystemExit(
                'Number of dimensions is different from 2 or 3. Exiting...')
    ...
    def circle(self):
        ...
        maskout = R < Rmax
        t = maskout * \
            (1-np.cos(np.pi*(R-Rmax-2*self.axial_apod)/self.axial_apod))/2.
    ...
    def transverse_apodization(self):
        ...
        if self.ndim == 2:
            ...
            window1D1 = (1.+np.cos(2*np.pi*(Nr-np.floor((self.nr -
                                                         2*self.transv_apod-1)/2))/(1+2*self.transv_apod)))/2.
            window1D2 = (1.+np.cos(2*np.pi*(Nc-np.floor((self.nc -
                                                         2*self.transv_apod-1)/2))/(1+2*self.transv_apod)))/2.
            ...
            Nr = np.fft.fftshift(np.arange(self.nr))
            Nc = np.fft.fftshift(np.arange(self.nc))
            window1D1 = (1.+np.cos(2*np.pi*(Ns-np.floor((self.ns -
                                                         2*self.transv_apod-1)/2))/(1+2*self.transv_apod)))/2.
            window1D2 = (1.+np.cos(2*np.pi*(Nr-np.floor((self.nr -
                                                         2*self.transv_apod-1)/2))/(1+2*self.transv_apod)))/2.
            window1D3 = (1.+np.cos(2*np.pi*(Nc-np.floor((self.nc -
                                                         2*self.transv_apod-1)/2))/(1+2*self.transv_apod)))/2.
            window1D1[self.transv_apod:-self.transv_apod] = 1
            window1D2[self.transv_apod:-self.transv_apod] = 1
            window1D3[self.transv_apod:-self.transv_apod] = 1
            window = [np.outer(window1D1, window1D2),
                      np.outer(window1D1, window1D3)]
        return window
    ...
    def fouriercorr(self):
        ...
        if self.ndim == 2:
            ...
            if self.apod_width == 0:
                ...
                circle3D = np.asarray(
                    [circular_region for ii in range(self.ns)])
                self.window = np.array([np.squeeze(circle3D[:, :, ii])*window3D[0]
                                        for ii in range(self.nc)]).swapaxes(0, 1).swapaxes(1, 2)
                self.window = np.array([np.squeeze(
                    self.window[:, ii, :])*window3D[1] for ii in range(self.nr)]).swapaxes(0, 1)
                print('Done')
        ...
        for ii in f:
            if self.ring_thick == 0 or self.ring_thick == 1:
                ...
                auxF1 = F1[(np.where((index >= (ii-self.ring_thick/2))
                                     & (index <= (ii+self.ring_thick/2))))]
                auxF2 = F2[(np.where((index >= (ii-self.ring_thick/2))
                                     & (index <= (ii+self.ring_thick/2))))]
        ...
        Tnum = (self.snrt + (2*np.sqrt(self.snrt) /
                             np.sqrt(npts+np.spacing(1)))+1/np.sqrt(npts))
...
class FSCPlot(FourierShellCorr):
    ...
    def plot(self):
        ...
        plt.legend()
        if self.snrt == 0.2071:
            plt.plot(self.f/self.fnyquist, self.T,
                     '--r', label='1/2 bit threshold')
            plt.legend()
        elif self.snrt == 0.5:
            plt.plot(self.f/self.fnyquist, self.T,
                     '--r', label='1 bit threshold')
            plt.legend()
        else:

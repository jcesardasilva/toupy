...
x0_1 = np.array([0.01, -0.01, values[:, 1].max(),
                 values[:, 0].mean(), sign_of_function*20.], dtype=float)
popt1, pcov1, infodict1, errmsg1, ier1 = leastsq(
    residuals1, x0_1, full_output=1, args=(values[:, 1], values[:, 0]))
x1_1 = popt1
...
linescan1, = ax1.plot(values[:, 0]-x1_1[3],
                      values[:, 1], 'ro-', mfc='none', mec='red', mew=2)
fitlinescan1, = ax1.plot(
    values[:, 0]-x1_1[3], model1(values[:, 0], *x1_1), 'b--', linewidth=3)
...
print('FWHM: {:.02f} nm'.format(
    np.sqrt(2*np.log(2))*np.abs(x1_1[-1])))  # *1e9))
print('Average position: {:.02f} nm'.format(x1_1[3]))  # *1e6))

# model 2
sign_of_function = np.sign(values[:, 1][0]-values[:, 1][-1])
x0_2 = np.array([0.01, -0.01, values[:, 1].max(),
                 values[:, 0].mean(), sign_of_function*20.], dtype=float)
popt2, pcov2, infodict2, errmsg2, ier2 = leastsq(
    residuals2, x0_2, full_output=1, args=(values[:, 1], values[:, 0]))
x1_2 = popt2

fig2 = plt.figure(2)
ax2 = fig2.add_subplot(111)
# motorscan-factor1,counterscan,'ro',mfc='none',mec='red',mew=2)
linescan2, = ax2.plot(values[:, 0]-x1_2[3],
                      values[:, 1], 'ro-', mfc='none', mec='red', mew=2)
fitlinescan2, = ax2.plot(
    values[:, 0]-x1_2[3], model2(values[:, 0], *x1_2), 'b--', linewidth=3)
...
print('=========== Fit with tanh =============')
print('FWHM: {:.02f} nm'.format(np.arccosh(
    1/np.sqrt(1/2.-x1_2[1]*np.abs(x1_2[4])/x1_2[2]))*2*np.abs(x1_2[4])))
print('FWHM (background removed): {:.02f} nm'.format(
    np.arccosh(1/np.sqrt(1/2.))*2*np.abs(x1_2[4])))
